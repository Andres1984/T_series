...
par(mfrow=c(2,1))
plot(Wheat$Date, Wheat$Last,type="l")
plot(WTI$Date, WTI$Settle,type="l")
...
wheat = Delt(Wheat$Last) # Rendimientos
wheat=wheat[-1,]
wti=Delt(WTI$Settle)
wti=wti[-1]
par(mfrow=c(2,1))
plot(Wheat$Date[-1], wheat, type="l", xlab="")
plot(WTI$Date[-1], wti,type="l", col="red",xlab="")
...
VaRP=qnorm(0.05)*  sd(wheat)*sqrt(20) # No olvide incluir el monto de la inversión
VaRP
...
options(digits = 3)
q=quantile(wheat,probs=c(0.025,0.05,0.1))
...
```{r, echo=FALSE, message=FALSE}
VaRH=q[2]*sqrt(20) # No olvide incluir el monto de la inversión
VaRH
q[2]
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
S = c(as.numeric(tail(Wheat$Last , n=1)))
mu = c(mean(wheat))
sigma = c(sd(wheat))
...
result=matrix(c(VaRP,VaRH,VaRSM0.5),nrow=3,ncol = 1)
colnames(result)=c("Trigo")
