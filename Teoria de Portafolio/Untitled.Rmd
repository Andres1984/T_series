...
1. Para $m \in R$ dada, se determina una cartera que tenga un rendimiento $m$ mínimo esperado y minimice el riesgo entre todas las carteras que también tengan un rendimiento esperado de al menos $m$ (problema de minimización de la varianza), respectivamente.
...
Se puede observar en la figura \@ref(fig:Precios)  que hay dos acciones BXP y AXP cuyos precios superan los cien dolares para el 2019 presentando grandes cambios en términos monetarios, lo que sugiere que el sector financiero de este portafolio es el más costoso y podría ser el mayor riesgo. Mientras que la empresa del sector utilities AES tiene precios accionarios muy bajos pero que se han incrementado de forma constante a partir de finales de 2017. Finalmente  se puede observar que GLW podría tener un comportamiento estable en precios con una variación entre 26 y 36 dolares durante el período observado, sin embargo, esto solo se puede corroborar con ayuda de las mediciones descriptivas.
...
kable(describe(rend), booktabs=TRUE,caption = "Estadísticas Descriptivas")
...
```{r, fig.width = 8, fig.height = 8,echo=FALSE, message=FALSE,warning=FALSE,echo=FALSE}
library(derivmkts)
par(mfrow=c(2,2))
binomplot(as.numeric(tail(BXP$BXP.Close, n=1)), 0, sd(bxp)/100*sqrt(5), (1.0231)^(1/52)-1,  4,0, nstep=4, american=TRUE, putopt=TRUE,
...
Usando el modelo binomial se realiza un pronóstico de los precios de cada acción para un mes dividido en cuatro semanas, se puede ver en cada una de las gráficas, que la varianza genera que el precio aumente y se aleje de la media a medida que avanzan las semanas. A mayor horizonte de tiempo, mayor sera el grado de dispersión con respecto al valor esperado. Entre las cuatro gráficas, la que pertenece a AXP presenta un mayor grado de dispersión mostrando que puede llegar a alcanzar variaciones hasta de  20 USD, mientras que AES parece no mostrar grandes cambios en un mes dado que su máxima variación podría ser de  2 USD.
...
kable(result, format = "markdown",caption = "Resultados")
```
...
La figura de precios de un instrumento financiero muestra el precio de una acción en verde cuando solo crece a una tasa $\mu$ a lo largo del tiempo, en este caso el efecto de la volatilidad y la distribución normal no genera un efecto directo sobre el precio de la acción y por esa razón solo se aprecia una linea verde sin variaciones y con un crecimiento constante.
...
Una vez que se generan las simulaciones con ayuda de la función caminatas, se logra obtener la figura de cada una de las acciones en donde se genera un tornado mostrando todos los posibles escenarios de cada uno de los precios de las acciones.
...
kable(CVPC, format = "markdown" ,caption = "Coeficiente de Variación")

```
El coeficiente de variación refleja el crecimiento de la volatilidad a medida que aumenta el tiempo. para la semana número cuatro se puede ver que los resultados con excepción de la acción de AES son similares a los que se obtuvieron con el modelo binomial. Mostrando una vez más que el proceso discreto y continuo son equivalentes.
...
library(knitr)
kable(resultados, format = "markdown",caption = "Resultados")



```

La tabla muestra el valor esperado y la volatilidad de cada activo para 20 días, se puede ver que la acción de la que menos se esperan incrementos y variaciones es BXP con una rentabilidad del 0.21% y una volatilidad del 0.14%, mientras que las demás acciones presentan rendimientos del por encima del 1% para los próximos 20 días. De acuerdo a la volatilidad GLW parece tener más posibilidades de tener variaciones de acuerdo al pronóstico realizado en la sección anterior, mientras que las acciones del sector financiero para que van a mostrar una relativa calma. En conclusión se espera que la mayor rentabilidad del portafolio sea la de GLW y la mínima sea la de BXP. Bajo estos parámetros se procede a obtener el mejor rendimiento dentro de ese intervalo, mínimizando el riesgo de portafolio.
...
La función globalMin.portfolio usa el vector de valores esperados y la matriz de varianza y covarianza para hacer el proceso de optimización que se presento a través de las matrices. El resultado de la ponderación del portafolio, es un retorno  del 0.88% con una volatilidad de 0.089%. 

Para obtener ese retorno a ese nivel de volatilidad, el modelo aconseja invertir 46.12% en GLW, el 33.61% en AXP, el 11.35% em AES y el 8.92% en GLW. 
...
Para poder observar mejor la relación entre riesgo y rentabilidad, se crean diez escenarios y se grafican de tal forma que se puedan observar cada una de las alternativas, se puede ver primero que a mayor rentabilidad mayor riesgo, segundo que si uno invierte en un solo activo el riesgo que se obtiene es mayor y la rentabilidad no es la misma que la que se puede obtener diversificando el portafolio.


La tabla adicional genera los resultados de las ponderaciones para cada uno de los portafolio donde el primero es el que ofrece una rentabilidad mayor el último es en el que se espera una mayor rentabilidad.
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
library(knitr)
kable(ef$weights, format = "markdown", caption = "Ponderación")

```
## Conclusión

La segunda fase tenía como objetivo introducir al estudiante en los procesos de incertidumbre; con ayuda del valor esperado y la varianza se buscaba que el estudiante estuviera en la capacidad de entender la naturaleza de los elementos que conforman la teoría de la probabilidad y aplicarlos a modelos financieros especializados en el pronóstico de precios.

Primero se implementó el modelo binomial que nace directamente de la función de distribución binomial, asignándole a una variable aleatoria que es el precio un factor de riesgo que es el rendimiento para obtener la probabilidad que permite modelar el precio de los activos financieros en diferentes escenarios. Una vez se obtienen los escenarios, se calcula el valor esperado y la varianza para analizar los posibles cambios del precio en un escenario discreto.
